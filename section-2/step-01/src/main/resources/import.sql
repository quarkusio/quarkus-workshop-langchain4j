...
INSERT INTO car_info (id, make, model, year, status) VALUES
    ...
    (nextval('car_info_id_seq'), 'Chevrolet', 'Malibu', EXTRACT(YEAR FROM CURRENT_DATE) - 5, 'AT_CLEANING'),
    (nextval('car_info_id_seq'), 'BMW', 'X5', EXTRACT(YEAR FROM CURRENT_DATE) - 1, 'AT_CLEANING'),
    (nextval('car_info_id_seq'), 'Mercedes-Benz', 'C-Class', EXTRACT(YEAR FROM CURRENT_DATE) - 3, 'RENTED'),
    (nextval('car_info_id_seq'), 'Audi', 'A4', EXTRACT(YEAR FROM CURRENT_DATE) - 2, 'RENTED'),
    (nextval('car_info_id_seq'), 'Nissan', 'Altima', EXTRACT(YEAR FROM CURRENT_DATE) - 4, 'AT_CLEANING'),
