...
**Store selection:** If a store applies, run `openspec store list --json` to find its id, then pass `--store <id>` on every command that accepts it (`new change`, `status`, `instructions`, `list`, `show`, `validate`, `archive`, `doctor`, `context`, `schemas`, `view`). Treat it as sticky for the session. Without a store, commands act on the nearest local `openspec/` root.
...
There's no required ending. Discovery might flow into a proposal, result in artifact updates, just provide clarity, or simply continue later. When things crystallize, you might offer a brief optional summary of what was figured out, the approach that emerged, and any open questions — but the thinking itself is often the value.
