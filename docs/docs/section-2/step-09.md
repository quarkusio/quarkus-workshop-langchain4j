...
## From Step 08 to Step 09: Adding Kubernetes Extensions

The code in `section-2/step-09/` is the same two-application system you built in Step 08 — the multi-agent system and the remote A2A pricing agent — with three new extensions added to each `pom.xml` to make them cluster-ready.
...
The code for this step lives in `section-2/step-09/`. It contains two Quarkus modules and the deployment resources:
...
The manifests in `section-2/step-09/kubernetes/` create the following resources, all inside the `miles-and-smiles` namespace.
...
You've now seen a complete picture of the Miles and Smiles agentic system — from a single agent with a tool in Step 01, through compositional workflows, supervisor orchestration, human approval, multimodal analysis, and distributed A2A communication in Step 08, to a production cluster deployment here. Head to the [Mastering Agentic Systems](conclusion.md) conclusion to reflect on everything you've built and the patterns you can carry into your own projects.
