...
class VehicleGuardrailConcurrencyTest {
    ...
    public static class ConcurrentProfile implements QuarkusTestProfile {
        ...
        public Set<Class<?>> getEnabledAlternatives() {
            return Set.of(ScriptedModel.class, ScriptedEnhancedModel.class);
        }

        @Alternative
        @Singleton
        @ModelName("enhancedModel")
        public static class ScriptedEnhancedModel implements ChatModel {
            ...
            public ChatResponse doChat(ChatRequest request) {
                String prompt = request.messages().reversed().stream().filter(UserMessage.class::isInstance)
                        ...
                        .filter(text -> text.contains("- Destination:") || text.contains("Vehicle:")
                                || text.contains("Current recommendation:"))
                ...
                if (prompt.contains("vehicle specialist")) {
                    ...
                    json = """
                            ...
                            """;
                } else if (prompt.contains("comfort evaluator") || prompt.contains("cost evaluator")
                        || prompt.contains("fuel efficiency evaluator")) {
                    json = "{\"score\":8.5,\"suggestions\":\"Vehicle is well suited\"}";
                } else if (prompt.contains("recommendation specialist")) {
                    String dest = prompt.lines()
                            .filter(line -> line.startsWith("Destination: "))
                            .findFirst().map(line -> line.substring("Destination: ".length()).trim())
                            .orElse("");
                    json = switch (dest) {
                        case "Adventure" -> "{\"type\":\"SUV\",\"model\":\"SUV; specific model subject to availability.\",\"reasoning\":\"Revised\"}";
                        case "Business" -> "{\"type\":\"Estate\",\"model\":\"Estate; specific model subject to availability.\",\"reasoning\":\"Revised\"}";
                        case "Couple" -> "{\"type\":\"Sports car\",\"model\":\"Ferrari Roma\",\"reasoning\":\"Revised\"}";
                        default -> "{\"type\":\"MPV\",\"model\":\"Family MPV; specific model subject to availability.\",\"reasoning\":\"Revised\"}";
                    };
