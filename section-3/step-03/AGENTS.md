...
When encountering a bug in a Quarkus extension or upstream library, **do not work around it in
application code**. Instead:

1. File an issue against the upstream project with a clear reproduction case.
2. Leave the code written in the correct/idiomatic way so it acts as a live reproducer.
3. Add a `TODO` comment in the relevant source file or `application.properties` referencing the
   upstream issue URL.
4. Document the issue here so future agents don't waste time re-investigating.

### quarkiverse/quarkus-flow#810 — `ContentBasedRouterEventsPublisher` uses `MutinyEmitter<byte[]>`

**Status:** Open, filed 2026-07-30.

`ContentBasedRouterEventsPublisher` (the internal publisher for the `flow-out` channel) is
hardwired to `MutinyEmitter<byte[]>` and serialises CloudEvents to JSON bytes itself via
`JsonFormat.serialize()`, bypassing SmallRye's CE support entirely. This means:

- `cloud-events=true` / `cloud-events-mode=structured` / `StringSerializer` on the `flow-out`
  channel have no effect — the publisher ignores them.
- In tests, `InMemorySink<String>` fails with `ClassCastException: [B cannot be cast to String`
  because the in-memory connector receives `byte[]`.
- `TripPlanStore`'s `@Incoming("flow-out-consumer")` must declare `CloudEvent` as its parameter
  type (using the Jackson-generated deserializer), which only works because
  `quarkus-smallrye-reactive-messaging-kafka` auto-generates a `CloudEvent` deserializer when it
  detects a `CloudEvent`-typed `@Incoming` method.

The `application.properties` is written with `StringSerializer` and `cloud-events-mode=structured`
(the correct idiomatic config) so this project acts as a reproducer until #810 is resolved.
