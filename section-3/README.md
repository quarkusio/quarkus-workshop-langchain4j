...
Steps `00` through `04` are successive snapshots of the trip planner. Each step directory is a complete, runnable Quarkus project. Later steps build on earlier ones; when you change shared behavior, propagate the fix forward through every implemented step that follows.
...
| **03** | Quarkus Flow + Kafka approval lifecycle, asynchronous API and UI |
| **04** | PostgreSQL persistence, Flow checkpoints, restart and restore |
...
- **Step 03** — `src/test/frontend/live.test.cjs` (real model; requires env vars)
- **Step 04** — `FlowRestartProbe` (three JVM phases against a disposable Postgres; see step-04 `README.md`)
...
3. After a fix in step `N`, apply the same fix to steps `N+1` … `04` if the affected files exist there unchanged.
...
GitHub Actions runs `./mvnw verify` per step in the build matrix (`section-3/step-00` … `step-04`). Each job should finish quickly because tests are scoped to that step only.
