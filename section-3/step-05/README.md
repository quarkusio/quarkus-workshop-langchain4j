# Step 05: Persistent trip approval

This step is Step 04 plus PostgreSQL persistence. The agent pipeline, pricing tool, output guardrails, Flow descriptor, REST envelopes, and browser UI are inherited. Vehicle and itinerary research run in parallel, followed by cost estimation. The plan has no separate tips field or tips agent. Rental rates are fictional; confirmation is a simulated booking and does not reserve a vehicle.

`quarkus-flow-jpa` stores workflow checkpoints and supplies Hibernate ORM with Panache transitively. `quarkus-jdbc-postgresql` supplies the database driver. These are the only added extensions. Versions and build plugins follow Step 04's `pom.xml`.
...
The persisted states are `planning`, `awaiting_approval`, `decision_submitted`, `confirmed`, `rejected`, and `failed`. The entity stores the reviewed plan, confirmation, safe error fields, and accepted decision. Status reads return the same `TripPlanStatus` envelope as Step 04. A later event cannot replace the reviewed plan or reopen a terminal trip. Event extension, payload identifiers, original request, and legal-transition checks are retained.
...
The default Surefire suite runs only Step 05 persistence tests. Guardrail and HTTP failure coverage stays in Step 02; Flow smoke tests stay in Step 04. The default run uses isolated PostgreSQL Dev Services with container reuse disabled and a fresh schema. Tests need no real API key or live model.
...
```bash
docker run --rm -d --name step05-restart-probe -p 127.0.0.1:55434:5432 -e POSTGRES_DB=restart -e POSTGRES_USER=restart -e POSTGRES_PASSWORD=restart postgres:18
...
docker stop step05-restart-probe
```
...
All three restart-probe phases passed against a dedicated PostgreSQL 18 container using Podman. Separate JVMs created workflow `01M2JYSRZ2RDXG2WGN8XFJV1BW`, restored its `waitApproval` task, confirmed it without another planning call, and recovered the confirmed record after a second restart. The probe container was stopped afterward. No live-model or real Kafka journey was run for Step 05; browser tests used intercepted responses.
